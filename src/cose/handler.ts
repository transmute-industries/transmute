...
export const handler = async function ({ positionals, values }: Arguments) {
  ...
  switch (operation) {
    case 'keygen': {
      ...
      const k = await cose.key.generate<Map<any, any>>(alg as any, 'application/cose-key')
      const thumbprint = k.get(2)
      if (verbose) {
        const message = `🔑 ${Buffer.from(thumbprint).toString('hex')}`
        debug(message)
      }
      if (output) {
        fs.writeFileSync(output, cose.cbor.encode(k))
      }
      const encoded = Buffer.from(cose.cbor.encode(k))
      if (env.github()) {
        const privateKey = k.get(-4)
        if (privateKey) {
          setSecret(Buffer.from(privateKey).toString('hex'))
        }
