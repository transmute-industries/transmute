...
import { IFSACommand, IFSAEvent, IUnmarshalledEsCommand, marshal, getFSAFromEventValues } from './Utils/Common'
...
export class EventStore {
  ...
  writeUnmarshalledEsCommand = async (
    ...
    let marshalledEvent = marshal(esEvent.eventType, esEvent.keyType, esEvent.valueType, esEvent.key, esEvent.value)
  ...
  writeFSA = async (eventStore: any, fromAddress: string, fsa: IFSACommand): Promise<IFSAEvent> => {
    if (_.isArray(fsa.payload)) {
      throw new Error('fsa.payload cannot be an array.')
    }
    ...
    if (_.isObject(fsa.payload) && payloadKeys.length > 1) {
      // CONVERT TO IPLD
      valueType = 'I'
    ...
    if (key === 'bytes32') {
      ...
      if (valueType === 'B') {
        if (value.length > 66)
          // check length
          throw new Error('solidity bytes32 type exceeded 32 bytes: ' + value.length + ' nybbles')
        if (!Common.isHex(value))
          // check hex chars only 0-f/F
          throw new Error('solidity bytes32 received invalid hex string: ' + value)
        // 32-Byte hex - 'the dog jumped'
      } else if (valueType === 'S') {
        if (value.length > 32)
          // check length of string
          throw new Error('solidity bytes32 type exceeded 32 bytes: ' + value.length + ' chars')
      } else throw new Error('Invalid value type - ' + valueType)
    } else if (key === 'address') {
      // address - 20 bytes
      if (valueType === 'A') {
        if (!Common.isHex(value))
          // check hex chars only 0-f/F
          throw new Error('solidity address received invalid hex string: ' + value)
        if (Common.formatHex(value).length !== 42) throw new Error('solidity address received invalid length: ' + value)
      } else throw new Error('Invalid value type - ' + valueType)
    }
