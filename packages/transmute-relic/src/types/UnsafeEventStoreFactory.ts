...
/**
 * UnsafeEventStoreFactory API
 */
export class UnsafeEventStoreFactory extends SoltsiceContract {
    static get Artifacts() { return require('../../node_modules/transmute-contracts/build/contracts/UnsafeEventStoreFactory.json'); }

    static get BytecodeHash() {
        // we need this before ctor, but artifacts are static and we cannot pass it to the base class, so need to generate
        let artifacts = UnsafeEventStoreFactory.Artifacts;
    ...
    static async New(deploymentParams: W3.TC.TxParams, ctorParams?: {}, w3?: W3, link?: SoltsiceContract[]): Promise<UnsafeEventStoreFactory> {
        let contract = new UnsafeEventStoreFactory(deploymentParams, ctorParams, w3, link);
        await contract._instancePromise;
        return contract;
    }

    static async At(address: string | object, w3?: W3): Promise<UnsafeEventStoreFactory> {
        let contract = new UnsafeEventStoreFactory(address, undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }
    ...
    protected constructor(
        ...
        super(
            w3,
            UnsafeEventStoreFactory.Artifacts,
            ctorParams ? [] : [],
            deploymentParams,
            link
        );
    ...
    */
    ...
    public killEventStore = Object.assign(
        // tslint:disable-next-line:max-line-length
        // tslint:disable-next-line:variable-name
        (_address: string, txParams?: W3.TC.TxParams): Promise<W3.TC.TransactionResult> => {
            return new Promise((resolve, reject) => {
                this._instance.killEventStore(_address, txParams || this._sendParams)
                    .then((res) => resolve(res))
                    .catch((err) => reject(err));
            });
        },
        {
            // tslint:disable-next-line:max-line-length
            // tslint:disable-next-line:variable-name
            sendTransaction: (_address: string, txParams?: W3.TC.TxParams): Promise<string> => {
                return new Promise((resolve, reject) => {
                    this._instance.killEventStore.sendTransaction(_address, txParams || this._sendParams)
                        .then((res) => resolve(res))
                        .catch((err) => reject(err));
                });
            }
        },
        {
            // tslint:disable-next-line:max-line-length
            // tslint:disable-next-line:variable-name
            data: (_address: string): Promise<string> => {
                return new Promise((resolve, reject) => {
                    resolve(this._instance.killEventStore.request(_address).params[0].data);
                });
            }
        },
        {
            // tslint:disable-next-line:max-line-length
            // tslint:disable-next-line:variable-name
            estimateGas: (_address: string): Promise<number> => {
                return new Promise((resolve, reject) => {
                    this._instance.killEventStore.estimateGas(_address).then((g) => resolve(g));
                });
            }
        });
