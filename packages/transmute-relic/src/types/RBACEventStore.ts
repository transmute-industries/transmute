 ...
 * RBACEventStore API
 */
export class RBACEventStore extends SoltsiceContract {
  static get Artifacts() {
    return require('../../node_modules/transmute-contracts/build/contracts/RBACEventStore.json')
  }

  static get BytecodeHash() {
    // we need this before ctor, but artifacts are static and we cannot pass it to the base class, so need to generate
    let artifacts = RBACEventStore.Artifacts
  ...
  ): Promise<RBACEventStore> {
    let contract = new RBACEventStore(deploymentParams, ctorParams, w3, link)
    await contract._instancePromise
    return contract
  }

  static async At(address: string | object, w3?: W3): Promise<RBACEventStore> {
    let contract = new RBACEventStore(address, undefined, w3, undefined)
    await contract._instancePromise
    return contract
  }
  ...
  protected constructor(
    ...
    // tslint:disable-next-line:max-line-length
    super(w3, RBACEventStore.Artifacts, ctorParams ? [] : [], deploymentParams, link)
  }
  /*
        Contract methods
    */
