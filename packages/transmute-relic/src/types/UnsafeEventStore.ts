...
/**
 * UnsafeEventStore API
 */
export class UnsafeEventStore extends SoltsiceContract {
    static get Artifacts() { return require('../../node_modules/transmute-contracts/build/contracts/UnsafeEventStore.json'); }

    static get BytecodeHash() {
        // we need this before ctor, but artifacts are static and we cannot pass it to the base class, so need to generate
        let artifacts = UnsafeEventStore.Artifacts;
    ...
    static async New(deploymentParams: W3.TC.TxParams, ctorParams?: {}, w3?: W3, link?: SoltsiceContract[]): Promise<UnsafeEventStore> {
        let contract = new UnsafeEventStore(deploymentParams, ctorParams, w3, link);
        await contract._instancePromise;
        return contract;
    }

    static async At(address: string | object, w3?: W3): Promise<UnsafeEventStore> {
        let contract = new UnsafeEventStore(address, undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }
