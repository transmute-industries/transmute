...
export class EventStoreFactory extends SoltsiceContract {
  static get Artifacts() {
    return require('../../node_modules/transmute-contracts/build/contracts/EventStoreFactory.json')
  }
  ...
  public setEventStoreWhitelist = Object.assign(
    ...
    // tslint:disable-next-line:variable-name
    (
      _eventStoreAddress: string,
      _whitelist: string[],
      txParams?: W3.TC.TxParams
    ): Promise<W3.TC.TransactionResult> => {
      return new Promise((resolve, reject) => {
        this._instance
          .setEventStoreWhitelist(_eventStoreAddress, _whitelist, txParams || this._sendParams)
          .then(res => resolve(res))
          .catch(err => reject(err))
      })
    },
    {
      ...
      sendTransaction: (
        ...
        return new Promise((resolve, reject) => {
          this._instance.setEventStoreWhitelist
            .sendTransaction(_eventStoreAddress, _whitelist, txParams || this._sendParams)
            .then(res => resolve(res))
            .catch(err => reject(err))
        })
      }
    },
    {
      ...
      data: (_eventStoreAddress: string, _whitelist: string[]): Promise<string> => {
        return new Promise((resolve, reject) => {
          resolve(
            this._instance.setEventStoreWhitelist.request(_eventStoreAddress, _whitelist).params[0]
              .data
          )
        })
      }
    },
    {
      ...
      estimateGas: (_eventStoreAddress: string, _whitelist: string[]): Promise<number> => {
        return new Promise((resolve, reject) => {
          this._instance.setEventStoreWhitelist
            .estimateGas(_eventStoreAddress, _whitelist)
            .then(g => resolve(g))
        })
      }
    }
  ...
  public killEventStore = Object.assign(
    ...
    (_eventStoreAddress: string, txParams?: W3.TC.TxParams): Promise<W3.TC.TransactionResult> => {
      return new Promise((resolve, reject) => {
        this._instance
          .killEventStore(_eventStoreAddress, txParams || this._sendParams)
          .then(res => resolve(res))
          .catch(err => reject(err))
      })
    },
      ...
      sendTransaction: (_eventStoreAddress: string, txParams?: W3.TC.TxParams): Promise<string> => {
        return new Promise((resolve, reject) => {
          this._instance.killEventStore
            .sendTransaction(_eventStoreAddress, txParams || this._sendParams)
            .then(res => resolve(res))
            .catch(err => reject(err))
        })
      }
