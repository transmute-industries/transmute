...
export class PackageManager extends SoltsiceContract {
    public static get Artifacts() { return require('../contracts/PackageManager.json'); }
    ...
    public static async New(deploymentParams: W3.TX.TxParams, ctorParams?: {}, w3?: W3, link?: SoltsiceContract[], privateKey?: string): Promise<PackageManager> {
        w3 = w3 || W3.Default;
        if (!privateKey) {
            ...
            let data = PackageManager.NewData(ctorParams, w3);
            let txHash = await w3.sendSignedTransaction(W3.zeroAddress, privateKey, data, deploymentParams);
            let txReceipt = await w3.waitTransactionReceipt(txHash);
            let rawAddress = txReceipt.contractAddress;
            let contract = await PackageManager.At(rawAddress, w3);
            return contract;
        }
    }

    public static async At(address: string | object, w3?: W3): Promise<PackageManager> {
        let contract = new PackageManager(address, undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }

    public static async Deployed(w3?: W3): Promise<PackageManager> {
        let contract = new PackageManager('', undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }
    ...
    public static NewData(ctorParams?: {}, w3?: W3): string {
        // tslint:disable-next-line:max-line-length
        let data = SoltsiceContract.NewDataImpl(w3, PackageManager.Artifacts, ctorParams ? [] : []);
        return data;
    }
