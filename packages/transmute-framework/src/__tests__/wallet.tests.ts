...
import { Relic, Utils, Factory, Store, W3, EventStore } from '../transmute-framework'
...
describe('Relic tests', () => {
  it('stores can be transfered between wallet and node addresses', async () => {
    ...
    txhash = await walletRelic.sendWei(walletAccounts[0], defaultAccounts[6], 150000000000000)
    expect(txhash).toBeDefined()
    ...
    await store.transferOwnership(walletAccounts[0], W3.TC.txParamsDefaultDeploy(storeOwner))
    ...
    expect(Utils.toChecksumAddress(storeOwner)).toEqual(Utils.toChecksumAddress(store2Owner))

    await store2.transferOwnership(defaultAccounts[2], W3.TC.txParamsDefaultDeploy(store2Owner))
