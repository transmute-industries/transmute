...
var _typeof = require("rollupPluginBabelHelpers").typeof;
...
IPFS.prototype.setProvider = function setProvider(provider) {
  if (_typeof(provider) !== 'object') {
    throw new Error("[ifpsjs] provider must be type Object, got '".concat(_typeof(provider), "'."));
  }
  ...
  self.requestBase = String("".concat(data.protocol, "://").concat(data.host, ":").concat(data.port).concat(data.base));
};
/**
...
IPFS.prototype.sendAsync = function sendAsync(opts, cb) {
  ...
  request.onreadystatechange = function () {
    if (request.readyState === 4 && request.timeout !== 1) {
      if (request.status !== 200) {
        callback(new Error("[ipfs-mini] status ".concat(request.status, ": ").concat(request.responseText)), null);
      } else {
        try {
          callback(null, options.jsonParse ? JSON.parse(request.responseText) : request.responseText);
        } catch (jsonError) {
          callback(new Error("[ipfs-mini] while parsing data: '".concat(String(request.responseText), "', error: ").concat(String(jsonError), " with provider: '").concat(self.requestBase, "'"), null));
        }
      }
    }
  };
  ...
  if (options.payload) {
    request.open('POST', "".concat(self.requestBase).concat(opts.uri).concat(pinningURI));
  } else {
    request.open('GET', "".concat(self.requestBase).concat(opts.uri).concat(pinningURI));
  }
  ...
  if (options.payload && options.boundary) {
    request.setRequestHeader('Content-Type', "multipart/form-data; boundary=".concat(options.boundary));
...
function createBoundary(data) {
  while (true) {
    var boundary = "----IPFSMini".concat(Math.random() * 100000, ".").concat(Math.random() * 100000);
...
IPFS.prototype.add = function addData(input, callback) {
  var data = _typeof(input) === 'object' && input.isBuffer ? input.toString('binary') : input;
  var boundary = createBoundary(data);
  var payload = "--".concat(boundary, "\r\nContent-Disposition: form-data; name=\"path\"\r\nContent-Type: application/octet-stream\r\n\r\n").concat(data, "\r\n--").concat(boundary, "--");
...
IPFS.prototype.stat = function cat(ipfsHash, callback) {
  ...
  self.sendAsync({
    jsonParse: true,
    uri: "/object/stat/".concat(ipfsHash)
  }, callback);
};
/**
