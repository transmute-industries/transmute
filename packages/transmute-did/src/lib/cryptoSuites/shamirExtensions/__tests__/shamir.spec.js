const shamirExtensions = require('../index');
...
describe('shamirExtensions', () => {
  describe('shatterKey', () => {
    it('should breakup a key in shares', async () => {
      expect.assertions(1);
      const shares2 = await shamirExtensions.shatterKey({
        key,
        shareNumber: 5,
        shareThreshold: 3,
      });
  ...
  describe('recoverKey', () => {
    it('should recover a key from shares', async () => {
      expect.assertions(1);
      const recoveredKey = await shamirExtensions.recoverKey({
