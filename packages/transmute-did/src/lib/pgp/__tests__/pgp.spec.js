...
describe("pgp", () => {
  describe("generateOpenPGPArmoredKeypair", () => {
    it("should generate a keypair", async () => {
      expect.assertions(2);
      let keypair = await pgp.generateOpenPGPArmoredKeypair({
        name,
        passphrase
      });
  ...
  describe("encryptMessage", () => {
    it("should encrypt a message", async () => {
      expect.assertions(1);
      const cipherText = await pgp.encryptMessage({
        message: testMessage,
  ...
  describe("decryptMessage", () => {
    it("should decrypt a message", async () => {
      expect.assertions(1);
      const plainText = await pgp.decryptMessage({
        message: cipherText,
  ...
  describe("sign", () => {
    it("should sign a message", async () => {
      expect.assertions(2);
      const signedMessage = await pgp.sign({
        message: testMessage,
        privateKey: testKeypair0.privateKey,
        passphrase
      });
      expect(signedMessage).toBeDefined();
      expect(await pgp.getMessagePayload(signedMessage)).toBe(testMessage);
    });
  });
