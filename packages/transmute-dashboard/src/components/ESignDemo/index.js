...
class ESignDemo extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  async componentWillMount() {
    ...
    this.setState({
      ...
      eventStore:
        eventStores.length > 0 ? eventStores[0] : this.state.eventStore,
      loading: false
    });

    if (!this.props.user.web3Account) {
      this.props.setWeb3Account(accounts[0]);
    }
  }
        ...
        <CardActions>
          <Button
            ...
            onClick={() => {
              let url =
                '/eventstorefactory/' +
                this.state.defaultEventStoreFactoryContractAddress;
              history.push(url);
            }}
          >
            Create EventStore
          ...
          <Button
            ...
            onClick={() => {
              let url = '/eventstore/' + this.state.eventStore + '/documents';
              this.props.go(url)
            }}
          >
            View Demo
...
export default connect(mapStateToProps, mapDispatchToProps)(
  withStyles(styles)(withAuth(ESignDemo))
);
