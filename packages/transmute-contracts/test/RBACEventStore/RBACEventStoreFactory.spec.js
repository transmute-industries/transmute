...
var RBACEventStoreFactory = artifacts.require(
  "./TransmuteFramework/RBACEventStoreFactory.sol"
);
var RBACEventStore = artifacts.require(
  "./TransmuteFramework/RBACEventStore.sol"
);
  ...
  contract("RBACEventStoreFactory", function(accounts) {
    ...
    it("createEventStore.call", async () => {
      ...
      assert.equal(
        ...
        "expected es contract address to match call"
      );

      let esAddress = fsa.payload.address;
      let es = await RBACEventStore.at(esAddress);
    ...
    //   let _tx = await factory.killEventStore(account1EventStoreAddresses[0], {
    //     from: accounts[0]
    //   });
    //   let event = _tx.logs[0].args;
    //   let fsa = getFSAFromEventArgs(event);
    ...
    //   );
    //   assert(
    //     _.includes(_addresses, account1EventStoreAddresses[1]),
      ...
      describe("owner can authorize other accounts to createEventStore", async () => {
        it("owner can make account 1 an admin", async () => {
          ...
          assert.equal(
            fsa.type,
            "AC_ROLE_ASSIGNED",
            "expect AC_ROLE_ASSIGNED event"
          );
          assert.equal(
            fsa.payload[accounts[1]],
            "admin",
            "expect account1 to be assigned admin"
          );
          // TODO: add more tests here...
        });

        it("owner can grant admin role create:any eventstore", async () => {
          tx = await factory.setGrant(
            "admin",
            "eventstore",
            "create:any",
            ["*"],
          ...
          assert(
            fsa.type,
            "AC_GRANT_WRITTEN",
            "expected event.type to be AC_GRANT_WRITTEN"
          );
        });
